# Optis（オプティス）— お小遣い・育成ガジェットアプリ

中学生向けのお小遣い管理＆マネーリテラシー育成アプリ。家計簿の入力を電子生命体
「Optis」の育成リソースに変換し、続けたくなる体験にする。
Next.js 16 + Prisma 7 + PostgreSQL (Neon) + Tailwind CSS。

## Optis 育成ガジェット機能

- **Optis Lab（ホーム）** — 常時アニメするキャラクター。タップで5種モーション＋吹き出し、
  端末シェイクで回転、EXPバー・レベル・信用スコアを表示
- **進化分岐** — 直近14日のNeeds/Wants比率で形態が変化（Needs≧60%でロジカル・ソリッド／
  Wants寄りでクリエイティブ・ネオン／中間でバランス・ハイブリッド）。レベルでステージも進化
- **高速スワイプ入力** — テンキー→金額を左右にスワイプしてNeeds/Wants仕分け→吸収エフェクトの3ステップ。
  記録ごとに経験値を獲得（Needsはボーナス）
- **デイリー・ジャックポット** — その日最初の記録でルーレット。NMD達成日は確変（レア確率UP）
- **ノーマネーデー & 不正検知** — 21時に放課後(16-19時)の支出有無を確認。「なし」申告で確変。
  後から該当時間帯の支出を足すとバックエンドが矛盾を検知し、24時間ステータス凍結（グリッチ＆経験値停止）
- **ウィークリー宝箱** — 月〜土の支出が週予算以下なら日曜にレアパーツ確定ドロップ
- **親子シークレット・ミッション** — 親が課題＋報酬（現金/レアパーツ）を送信、子が「緊急電文」で受信。
  クリア報告→親承認で報酬を自動付与（現金は収入計上）
- **Dark Web Mode（裏モード）** — 21:00〜24:00にキャラのコアを3秒長押しで解放。サイバーパンクUIで
  「物欲アーカイブ（我慢して貯めた総額）」「ステータスハッキング（次進化までの正確なEXP）」
  「パーツ装備（カスタマイズ）」を表示
