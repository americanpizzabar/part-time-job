...
import { PARTS, RARITY_META, Part, BRAIN_META, BrainType } from "@/lib/optis";
import GuildPanel from "@/components/GuildPanel";

interface DarkWebPanelProps {
  optis: { level: number; intoLevel: number; needed: number; experience: number; creditScore: number; gcoins?: number };
  onExit: () => void;
  onChanged: () => void;
}
...
interface BrainData {
  brainType: BrainType;
  stats: { needsRatio: number; impulseRate: number; avgGap: number; txCount: number } | null;
}
        ...
        <div className="grid grid-cols-3 gap-1.5 mb-5">
          {TABS.map(([t, l]) => (
            <button key={t} onClick={() => setTab(t)}
              className={`py-1 text-[10px] font-mono rounded border transition-colors ${tab === t ? "bg-cyan-900/60 border-cyan-400 text-cyan-200" : "border-cyan-900/40 text-cyan-700 hover:text-cyan-500"}`}>
              {l}
            </button>
          ))}
        </div>
                  ...
                  {topCats.filter(c => c.perWeekAmount > 0).map(cat => (
                    <div key={cat.name} className="bg-black/30 border border-fuchsia-900/40 rounded-lg p-2.5">
                      ...
                      <div className="text-emerald-300 text-xs font-mono mt-0.5">
                        → 週 +{formatJPY(cat.avgPerOccurrence)} 節約 → {daysToGoal(cat.avgPerOccurrence * 0.5)}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="border border-yellow-700/40 rounded-xl p-4 bg-black/40">
              ...
              <div className="space-y-2">
                ...
                <div>
                  ...
                  <input type="range" min={5} max={70} value={phDrop} onChange={e => setPhDrop(e.target.value)}
                    className="w-full accent-yellow-400" />
                  <div className="text-center text-yellow-400 font-mono text-sm">{phDrop}%</div>
                </div>
                {phNow && Number(phNow) > 0 && (() => {
                  ...
                  return (
                    <div className="bg-black/30 border border-yellow-800/40 rounded-lg p-3 font-mono">
                      {/* 天秤UI */}
                      <div className="flex items-end justify-center gap-4 mb-2 relative">
                      ...
                      <div className="text-center text-emerald-300 text-sm">節約: <span className="text-xl font-bold">{formatJPY(saved)}</span></div>
                      <div className="text-center text-yellow-700 text-[11px] mt-1">
                        この {formatJPY(saved)} でできること → 週予算 {sim?.weeklyBudget ? `${Math.round(saved / sim.weeklyBudget * 10) / 10} 週分` : "確認不能"}
                      </div>
                    </div>
                  );
                })()}
                ...
                <>
                  ...
                  {brain.stats && (
                    <div className="grid grid-cols-2 gap-3 font-mono text-xs">
                      <div>
                        <div className="text-cyan-600 text-[10px]">NEEDS_RATIO</div>
                        <div className="text-cyan-200">{brain.stats.needsRatio}%</div>
                      </div>
                      <div>
                        <div className="text-cyan-600 text-[10px]">IMPULSE_RATE</div>
                        <div className={brain.stats.impulseRate > 30 ? "text-red-300" : "text-cyan-200"}>{brain.stats.impulseRate}%</div>
                      </div>
                      <div>
                        <div className="text-cyan-600 text-[10px]">AVG_GAP_DAYS</div>
                        <div className="text-cyan-200">{brain.stats.avgGap}日</div>
                      </div>
                      <div>
                        <div className="text-cyan-600 text-[10px]">TX_COUNT_30D</div>
                        <div className="text-cyan-200">{brain.stats.txCount}件</div>
                      </div>
                    </div>
                  )}
                    ...
                    {BRAIN_META[brain.brainType] && (
                      <div className="space-y-1">
                        {[
                          ...
                          <div key={i} className="text-[11px] text-cyan-200/80 bg-black/20 rounded p-2 italic">{line}</div>
                        ))}
                      </div>
                    )}
                  </div>
        ...
        {tab === "bank" && (
          <div className="space-y-3">
            <div className="border border-yellow-600/40 rounded-xl p-4 bg-black/40">
              ...
              {depositMsg && (
                <div className="text-xs text-yellow-200 bg-yellow-900/30 border border-yellow-700/40 rounded-lg p-2 mb-3">
                  {depositMsg}
                </div>
              )}
                      ...
                      <div key={d.id} className="bg-black/30 border border-yellow-800/40 rounded-lg p-3 font-mono">
                        <div className="flex justify-between items-start">
                          ...
                          <div className={`text-[11px] px-2 py-0.5 rounded ${
                            ...
                            d.maturityDate <= today ? "bg-emerald-900/50 text-emerald-300 animate-pulse" :
                            "bg-yellow-900/30 text-yellow-500"}`}>
                            {d.status === "MATURED" ? "受取済" : d.status === "WITHDRAWN" ? "引出済" : d.maturityDate <= today ? "満期！" : "運用中"}
                          </div>
                        </div>
                        {d.status === "ACTIVE" && d.maturityDate > today && (
                          <button onClick={() => withdraw(d.id)}
                            className="mt-2 text-[11px] text-red-500 border border-red-800/40 rounded px-2 py-0.5">
                            早期引き出し(利息没収)
                          </button>
                        )}
                      </div>
